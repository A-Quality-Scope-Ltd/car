...
class _CodePhoneState extends State<CodePhone> {
  ...
  verifyOTP(context) async {
    ...
    try {
      ...
      if (widget.phoneNumber == phone) {
        Navigator.pushReplacement(
            context, MaterialPageRoute(builder: (context) => const DashBord()));
      } else if (dataProvide.userType) {
        try {
          var v = await FirebaseAppHelper()
              .getSellerData(phone: widget.phoneNumber!);
          if (v['allow']) {
            SharedPreferencesSignup()
                .saveData(key: SharedPreferencesKeys.userType, value: 'بائع');
            SharedPreferencesSignup().saveData(
                key: SharedPreferencesKeys.phone, value: widget.phoneNumber!);
            Provider.of<ProviderApp>(context, listen: false)
                .changeNotifierDataSeller(
                    accountInfoData: SellerRegistrationInfo.fromJson(v));
            Provider.of<ProviderApp>(context, listen: false)
                .changeNotifierID(phone: widget.phoneNumber!, userType: true);
            Navigator.pushReplacement(
              context,
          ...
          Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                  builder: (context) => RegisterNewSeller(
                        phone: widget.phoneNumber!,
                      )));
        }
      } else {
        Provider.of<ProviderApp>(context, listen: false)
            .changeNotifierID(phone: widget.phoneNumber!, userType: false);
        SharedPreferencesSignup()
            .saveData(key: SharedPreferencesKeys.userType, value: 'مستخدم');
        SharedPreferencesSignup().saveData(
            key: SharedPreferencesKeys.phone, value: widget.phoneNumber!);
        AccountRegistrationInfo setUser =
            AccountRegistrationInfo(phone: widget.phoneNumber!);
        FirebaseAppHelper()
            .setUserData(phone: widget.phoneNumber!, setUser: setUser.toMap());
      ...
      smsCode = '';
      customSnackBar(context: context, text: ' الرمز غير صحيح');
    }
  }
}
